...
contract ERC721TokenWrapped is ERC721Enumerable {
  ...
  modifier onlyBridge() {
    require(msg.sender == bridgeAddress, "TokenWrapped::onlyBridge: Not BTCLayer2Bridge");
    _;
  }
  ...
  function mint(address to, uint256 tokenId, string memory inscriptionId) external virtual onlyBridge {
    //adjust exist
  ...
  function burn(address sender, uint256 tokenId) external onlyBridge returns (string memory) {
    require(ownerOf(tokenId) == sender, "Illegal permissions");
